...
/* verify that the mxArray contains a float matrix */

void checkmatrix(const mxArray *param, char *fname, char *pname)
{
  char errmsg[100];
  sprintf(errmsg, "%.15s requires that %.25s be a float matrix.", fname, pname);
...
/* verify that the mxArray contains a 1-D float vector */

void checkvector(const mxArray *param, char *fname, char *pname)
{
  char errmsg[100];
  sprintf(errmsg, "%.15s requires that %.25s be a float vector.", fname, pname);
...
/* verify that the mxArray contains a float scalar */

void checkscalar(const mxArray *param, char *fname, char *pname)
{
  char errmsg[100];
  sprintf(errmsg, "%.15s requires that %.25s be a float scalar.", fname, pname);
  if (!mxIsDouble(param) || mxIsComplex(param) || mxGetNumberOfDimensions(param)>2 || 
