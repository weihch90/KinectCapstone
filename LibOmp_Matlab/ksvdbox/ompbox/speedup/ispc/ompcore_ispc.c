...
void addproftime2(profdata *pd, int comptype) {}

extern void matT_vec_ispc(const double alpha, double *A, double *x, double *y, const int64_t n, const int64_t m);
extern void mat_vec_ispc(const double alpha, double *A, double *x, double *y, const int64_t n, const int64_t m, double *tmp);
                 ...
                 int T, double eps, int gamma_mode, int profile, double msg_delta, int erroromp)
{
  ...
  tmp_n = (double*)mxMalloc( (n)*sizeof(double) );
  tmp_m = (double*)mxMalloc( (m)*sizeof(double) );
  tmp_L = (double*)mxMalloc( (L)*sizeof(double) );
  ...
  for (signum=0; signum<L; ++signum) {
    ...
    while (resnorm>eps2 && i<T) {
      ...
      if (i==0) {
        *Lchol = 1;
        ...
        backsubst_L_ispc( Lchol, tempvec1, tempvec2, n, i);   /* compute tempvec2 = Lchol \ tempvec1 */
      ...
      cholsolve_L_ispc( Lchol, tempvec1, c, n, i, tmp_L);                     /* solve LL'c = tempvec1 for c */
      addproftime2(&pd, COMPCOEF_TIME);
      ...
      if (standardomp) {
        ...
        if (erroromp) {
          ...
          resnorm = dotprod_ispc(r, r, n);
          addproftime2(&pd, UPDATE_RESNORM_TIME);
        }
      }
      else {
        if (0)
        ...
        addproftime2(&pd, UPDATE_DtR_TIME);
        ...
        if (erroromp) {
          ...
          delta = dotprod_ispc(c,tempvec2,i);               /* compute c'*tempvec2 */
    ...
    if (gamma_mode == FULL_GAMMA) {    /* write the coefs in c to their correct positions in gamma */
      ...
      quicksort(ind,c,i);
      addproftime2(&pd, INDEXSORT_TIME);
