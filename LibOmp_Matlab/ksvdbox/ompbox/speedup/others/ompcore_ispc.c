...
export void ompcore(uniform double D[], uniform double x[], uniform double DtX[], uniform double XtX[], uniform double G[], mwSize n, mwSize m, mwSize L, int T, double eps, int gamma_mode, int profile, double msg_delta, int erroromp, uniform mxArray* Gamma)
{
...
//    Gamma = mxCreateDoubleMatrix(m, L, mxREAL);
 ...
 //   Gamma = mxCreateSparse(m, L, allocated_coefs, mxREAL);
  ...
  for (uniform signum_c=0; signum_c<L; signum_c+=programCount) {
    
    signum=signum_c+programIndex;
